...
class inversion:
    ...
    knowns and unknowns:
    La_lz, Sm_lz, Dy_lz, Yb_lz (concentrations of La, Sm, Dy, and Yb in mantle lherzolite)
    La_px, Sm_px, Dy_px, Yb_px (concentrations of La, Sm, Dy, and Yb in mantle pyroxenite
    ...
    def __init__(self, lithologies, data, knowns, unknowns, DeltaP=0.004, SpreadingCentre=True,
        ...
        if self.Traces is True:
            if ('La_Sm' and 'Dy_Yb') in self.data.keys():
                variables.extend(['La_lz', 'Sm_lz', 'Dy_lz', 'Yb_lz'])
                if self.MORBmelts is True:
                    variables.extend(['MORBmelts'])
                else:
                    variables.extend(['La_px', 'Sm_px', 'Dy_px', 'Yb_px'])
            else:
                sys.exit('Please enter \'La/Sm\', and \'Dy/Yb\' into the data and the '
        ...
        if run_model is True:
            ...
            if self.Traces is True:
                if self.MORBmelts is True:
                    ...
                    Sm_px = 0.239 / (x[self.var_list.index('MORBmelts')] * (1 - 0.045) + 0.045)
                    ...
                    results.calculateChemistry(
                        elements={'lz': {'La': x[self.var_list.index('La_lz')],
                                         'Sm': x[self.var_list.index('Sm_lz')],
                    ...
                    results.calculateChemistry(
                        elements={'lz': {'La': x[self.var_list.index('La_lz')],
                                  ...
                                  'px': {'La': x[self.var_list.index('La_px')],
                                         'Sm': x[self.var_list.index('Sm_px')],
            ...
            if self.Traces is True:
                La_Sm_ratio = geosetting.chemistry['La'] / geosetting.chemistry['Sm']
                Dy_Yb_ratio = geosetting.chemistry['Dy'] / geosetting.chemistry['Yb']

                likelihood = (
                    likelihood + (-0.5 * (np.log(2 * np.pi * self.data['La_Sm'][1]**2)) -
                                  (self.data['La_Sm'][0] - La_Sm_ratio)**2 /
                                  (2 * self.data['La_Sm'][1]**2)))
                likelihood = (
